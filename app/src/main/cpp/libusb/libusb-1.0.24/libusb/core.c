 ...
 * libusb_ref_device() and orion_libusb_unref_device(). A device is destroyed when
...
static void discovered_devs_free(struct discovered_devs *discdevs)
{
	...
	for (i = 0; i < discdevs->len; i++)
		orion_libusb_unref_device(discdevs->devices[i]);
 ...
 */
void API_EXPORTED orion_libusb_unref_device(libusb_device *dev)
{
	int refcnt;
	...
	if (refcnt == 0) {
		...
		orion_libusb_unref_device(dev->parent_dev);
	...
	struct libusb_device_handle *dev_handle)
{
	...
	orion_libusb_unref_device(dev_handle->dev);
